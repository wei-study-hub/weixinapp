...
public class WxpubOAuth {
	 ...
	 *            微信公众号应用密钥（注意保密）
	 * @param code
	 *            授权code, 通过调用WxpubOAuth.createOauthUrlForCode来获取
	 ...
	 * @param moreInfo
	 *            FALSE 不弹出授权页面,直接跳转,这个只能拿到用户openid TRUE 弹出授权页面,这个可以通过 openid
	 *            拿到昵称、性别、所在地，
	...
	public static String createOauthUrlForCode(String appId, String redirectUrl, boolean moreInfo)
		...
		data.put("state", "123#wechat_redirect");
		StringBuilder sb = new StringBuilder();
		sb.append(URLEncoder.encode("appid", "UTF-8")).append('=')
				.append(URLEncoder.encode(data.get("appid"), "UTF-8"));
		sb.append('&');
		sb.append(URLEncoder.encode("redirect_uri", "UTF-8")).append('=')
				.append(URLEncoder.encode(data.get("redirect_uri"),"UTF-8"));
		sb.append('&');
		sb.append(URLEncoder.encode("response_type", "UTF-8")).append('=')
				.append(URLEncoder.encode(data.get("response_type"), "UTF-8"));
		sb.append('&');
		sb.append(URLEncoder.encode("scope", "UTF-8")).append('=')
				.append(URLEncoder.encode(data.get("scope"), "UTF-8"));
		sb.append('&');
		sb.append(URLEncoder.encode("state", "UTF-8")).append('=')
				.append(data.get("state"));

		// String queryString = WxpubOAuth.httpBuildQuery(data);
		System.out.println("---->>>>>333");
		return "https://open.weixin.qq.com/connect/oauth2/authorize?" + sb.toString();
	}
	...
	private static String createOauthUrlForOpenid(String appId, String appSecret, String code)
		...
		sb.append(URLEncoder.encode("appid", "UTF-8")).append('=')
				.append(URLEncoder.encode(data.get("appid"), "UTF-8"));
		sb.append('&');
		sb.append(URLEncoder.encode("secret", "UTF-8")).append('=')
				.append(URLEncoder.encode(data.get("secret"), "UTF-8"));
		sb.append('&');
		sb.append(URLEncoder.encode("code", "UTF-8")).append('=').append(URLEncoder.encode(data.get("code"), "UTF-8"));
		sb.append('&');
		sb.append(URLEncoder.encode("grant_type", "UTF-8")).append('=')
				.append(URLEncoder.encode(data.get("grant_type"), "UTF-8"));

		//String queryString = WxpubOAuth.httpBuildQuery(data);

		return "https://api.weixin.qq.com/sns/oauth2/access_token?" + sb.toString();
	}
	...
	public static String getSignature(String charge, String jsapiTicket, String url) {
		...
		String signature = "";
		// 注意这里参数名必须全部小写，且必须有序
		String string1 = "jsapi_ticket=" + jsapiTicket + "&noncestr=" + wx_pub.get("nonceStr").getAsString()
				+ "&timestamp=" + wx_pub.get("timeStamp").getAsString() + "&url=" + url;
		try {
